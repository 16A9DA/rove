...
def test_registry_lists_all_browser_tools() -> None:
    names = {tool.name for tool in browser_registry(FakeBrowserController()).list()}
    assert names == {"open_url", "get_text", "screenshot", "click", "type", "scroll", "keypress", "wait", "finish"}


def test_all_browser_tools_are_low_risk() -> None:
    assert all(tool.risk_level == ToolRiskLevel.LOW for tool in browser_registry(FakeBrowserController()).list())


def test_open_url_launches_and_navigates() -> None:
    browser = FakeBrowserController()
    result = ToolExecutor(browser_registry(browser)).execute("id1", "open_url", json.dumps({"url": "https://example.com"}))
...
def test_click_targets_browser() -> None:
    browser = FakeBrowserController()
    result = ToolExecutor(browser_registry(browser)).execute("id1", "click", json.dumps({"x": 5, "y": 6}))
...
def test_get_text_returns_browser_text() -> None:
    browser = FakeBrowserController()
    result = ToolExecutor(browser_registry(browser)).execute("id1", "get_text", "{}")
...
def test_screenshot_strips_image_onto_toolresult_not_content() -> None:
    browser = FakeBrowserController()
    result = ToolExecutor(browser_registry(browser)).execute("id1", "screenshot", "{}")
