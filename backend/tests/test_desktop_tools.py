...
def test_registry_lists_all_desktop_tools() -> None:
    names = {tool.name for tool in desktop_registry(FakeNativeController()).list()}
    assert names == {"open_application", "focus_application", "screenshot", "click", "type", "scroll", "keypress", "wait", "finish"}


def test_all_desktop_tools_are_low_risk() -> None:
    assert all(tool.risk_level == ToolRiskLevel.LOW for tool in desktop_registry(FakeNativeController()).list())


def test_open_application_activates_native() -> None:
    native = FakeNativeController()
    result = ToolExecutor(desktop_registry(native)).execute("id1", "open_application", json.dumps({"name": "Finder"}))
...
def test_screenshot_strips_image_onto_toolresult_not_content() -> None:
    native = FakeNativeController()
    result = ToolExecutor(desktop_registry(native)).execute("id1", "screenshot", "{}")
